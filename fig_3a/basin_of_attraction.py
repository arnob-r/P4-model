### faster version..... using numba and jit

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Code is for calculating basin of attraction.
"""
...
except Exception:  # fallback if tqdm isn't available
    class tqdm:
...
def load_adjacency(fname: str, shape: tuple, allow_transpose: bool = True) -> np.ndarray:
    ...
    if allow_transpose and M.T.shape == shape:
        print(f"[note] {fname} was {M.shape}; using its transpose {M.T.shape} to match expected {shape}.")
        return M.T
    raise ValueError(f"{fname}: expected shape {shape} or its transpose, found {M.shape}")
...
def integrate_once_jit(A0, P0, S0, params,
    ...
    for _ in range(mm):
        ...
        A += (h / 6.0) * (k1A + 2.0 * (k2A + k3A) + k4A)
        P += (h / 6.0) * (k1P + 2.0 * (k2P + k3P) + k4P)
        S += (h / 6.0) * (k1S + 2.0 * (k2S + k3S) + k4S)
    return A, P, S
...
def _run_combo(combo):
    Ai0, Pj0, Sk0 = combo
    n1 = _G_N1
    n2 = _G_CACHES["Adj1"].shape[1]
    n3 = _G_CACHES["Adj2"].shape[1]

    A0 = np.full(n1, Ai0, dtype=np.float64)
    P0 = np.full(n2, Pj0, dtype=np.float64)
    S0 = np.full(n3, Sk0, dtype=np.float64)

    A_fin, P_fin, S_fin = integrate_once_jit(
        A0, P0, S0, _G_PARAMS,
    ...
    row = [Ai0, Pj0, Sk0] + A_fin.tolist()
    return " ".join("{:.5f}".format(x) for x in row)

# -------------------- Basin sweep --------------------
def sweep_node_triplet_jit(out_path: str = "Pollinator_basin_jit.txt",
                           iA: int = 0, iP: int = 0, iS: int = 0,
                           levels: int = 10,
                           mm: int = 200000, h: float = 0.01,
...
#################################################      
    ...
    beta1 = build_beta(n1, bii=1.0, bij=0.1)
    beta2 = build_beta(n2, bii=1.0, bij=0.1)
    beta3 = build_beta(n3, bii=1.0, bij=0.1)
    ...
    params = np.array([alpha, kappa, gamma0, xi0, mA, mS, f, ubar, mu, tt, hh], dtype=np.float64)
    caches = dict(beta1=beta1, beta2=beta2, beta3=beta3,
                  Adj1=Adj1, Adj2=Adj2, D1A=D1A, D1P=D1P, D2P=D2P, D2S=D2S)

    grid_vals = np.linspace(0.001, 1.0, levels)                                ######### Change
    ...
    chunksize = max(1, min(1000, int(math.ceil(float(num_runs) / (n_workers * 4)))))

    with out_path.open("w") as fout, ctx.Pool(
        processes=n_workers,
        initializer=_init_worker,
        initargs=(params, caches, h, mm, n1),
    ) as pool:

        iterator = pool.imap_unordered(_run_combo, combos, chunksize=chunksize)
        if show_progress:
            iterator = tqdm(iterator, total=num_runs, dynamic_ncols=True,
                            desc="Sweeping basin (JIT)", smoothing=0.1, leave=True)
    ...
    print("[timing] JIT Wall-clock runtime: %.3f s (%.3f min) with %d workers, chunksize=%d"
          % (wall_seconds, wall_seconds/60.0, n_workers, chunksize))

# -------------------- Entry point --------------------
if __name__ == "__main__":
    sweep_node_triplet_jit(out_path="Pollinator_basin_A_xu_0.1_f_0.1_gamma0_1.8.txt",       # Change
                           levels=30, mm=100000, h=0.01,                      
                           show_progress=True)
