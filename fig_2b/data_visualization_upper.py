...
# === Load data ===
data1 = loadtxt_first("Pollinator_bif_fwd_f0.1_0.2.txt", "../Pollinator_bif_f_0.1_0.2.txt")
data2 = loadtxt_first("Pollinator_bif_fwd_f0.1_0.5.txt")
data3 = loadtxt_first("Pollinator_bif_fwd_f0.1_0.8.txt")
...
for ax, data, title in zip(axes, datasets, titles):
    ...
    # Axis labels & titles
    ax.set_xlabel(r"$\mathbf{x_u}$", fontsize=32, fontweight='bold')
...
# Shared y-label
axes[0].set_ylabel(r"$\mathbf{A}$", fontsize=32, fontweight='bold')
...
plt.savefig(BASE_DIR / "Pollinator_bifurcation_panels_f_fixed.png", dpi=600, bbox_inches='tight')

plt.show()


#data = np.loadtxt('Pollinator_bif_fwd_xu0.1_0.2.txt')
#data = np.loadtxt('Pollinator_bif.txt')
#X= data[:,0]
#Y= data[:,1:23]
#plt.plot(X, Y,'b.-')
